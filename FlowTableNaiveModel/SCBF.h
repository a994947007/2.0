...
public:
	...
	bool init_h3_matrix();					//初始化哈希函数矩阵
	bool filter_insert(SCFlow &);		//插入一条流到过滤器
	bool filter_query(const FlowID &);		//查找集合中是否存在可匹配的流ID
	bool filter_delete(const SCFlow &);		//在SCBF中删除对应的流ID
	ULONG h3_function(const ULONG h3_row, const ULONG h3_hash, const int matrix_index);//h3哈希函数
	SCNode * lookup_spare_scNode();			//查找流表项数目小于最佳容量的结点
	virtual ~SCBF();
};
